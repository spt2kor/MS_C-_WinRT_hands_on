...
void TestExperimentalCode()
{
    winrt::MyUWPApp::VAObservable vaObservable;
    Log("<Reg_T1>");

    event_token token1 = vaObservable.NotifyOnDataChange(
        [](const winrt::MyUWPApp::PODData& _value) {
            Log(string("[T1]received:" + to_string(_value.m_msg())));
        }
    );
    ...
    data1.m_msg(L"_@Notify_T1");
    vaObservable.SetData(data1);
    */
    Log("<remove T1>");
    Sleep(2000);
    vaObservable.NotifyOnDataChange(token1);

    Sleep(2000);

   /* Log("set the data completed");

    Log("going to set the data T1_1 ");

    winrt::MyUWPApp::PODData data2;
    data2.m_msg(L"_@Notify_T1_2");
